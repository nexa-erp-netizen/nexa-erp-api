...
router.get("/diagnostico/:clienteId", async (req, res) => { const clienteId = Number(req.params.clienteId); const [cliente, cfg, servicos, certificado] = await Promise.all([Cliente.findByPk(clienteId), NFSeConfiguracao.findOne({ where: { clienteId } }), ServicoNFSe.count({ where: { clienteId, ativo: true } }), CredencialAcessoFiscal.findOne({ where: { clienteId, metodo: "A1", ativo: true }, order: [["updatedAt", "DESC"]] })]); if (!cliente) return res.status(404).json({ message: "Cliente não encontrado" }); const p = []; if (!cliente.cnpj) p.push("Informe o CNPJ do prestador."); if (!cfg?.inscricaoMunicipal && !cliente.inscricaoMunicipal) p.push("Informe a inscrição municipal do prestador, quando exigida pelo município."); if (!cfg?.municipioIbge) p.push("Informe o código IBGE do município do prestador."); if (!cfg?.regimeTributario) p.push("Selecione o regime tributário do prestador."); if (!servicos) p.push("Cadastre ao menos um serviço fiscal."); if (!certificado?.arquivoCriptografado || !certificado?.segredoCriptografado) p.push("Envie o certificado A1 e a senha no Cofre de acessos fiscais."); p.push("A transmissão será liberada após validar a DPS assinada na API de produção restrita."); res.json({ prontoParaRascunho: p.length <= 2, prontoParaEmitir: p.length === 1, ambiente: "homologacao", certificadoA1: certificado ? { id: certificado.id, nomeArquivo: certificado.nomeArquivo, configurado: Boolean(certificado.arquivoCriptografado && certificado.segredoCriptografado) } : null, pendencias: p, aviso: "Inscrição estadual não é exigida neste módulo." }) })
